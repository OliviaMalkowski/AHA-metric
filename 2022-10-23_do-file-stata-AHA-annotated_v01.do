...
replace guralnik = 4 if gaittime < 3.2
...
replace guralnik = 0 if (mmalone==3 | inlist(mmhss,2,3,4) | mmsaf==2  | mmwala==2 | inlist(mmtrya,2,3)) & indager >=60
...
* [b4] Whether respondent felt lonely must of the time during the past week
...
* Assign the number 0 if the participant reported having an alcoholic drink once a month or less during the last 12 month
...
* Assign the number 2 if the participant reported they 
...
* Save dataset with a new name
save baseline.dta
* Generate a new variable equal to the sum of variables with available "non-missing" data for each participant
egen nmcount = rownonmiss(_all), strok
tab nmcount
* Drop observation if > 25% missing values across the 64 variables of interest (idauniq and wave data are complete for all observations)
drop if nmcount<49
* Drop unnecessary variable
drop nmcount
* Count total number of participants
unique idauniq
* 7,660 individuals
* Generate a new variable equal to the sum of variables with missing data for each participant
...
* Save dataset with a new name
save MLIRTGMM.dta
* One-to-one merge of data in memory with MLIRTdata.dta (exported from RStudio) on participant ID
...
* 8,670 individuals
tab total
* Baseline characteristics of the complete-case sample used for the GMM analyses versus the sample excluded due to missing data (unweighted)
...
* Summarise posterior probabilities for each latent class

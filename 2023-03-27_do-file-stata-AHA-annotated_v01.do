********************************************************************************************************************************************************************
***SYNTAX FOR "Socio-economic status and trajectories of a novel multidimensional metric of Active and Healthy Ageing: the English Longitudinal Study of Ageing"***
********************************************************************************************************************************************************************
...
* Assign the number 1 if the participant completed five sit-to-stands in 16.7 seconds or more but less than or equal to 60 seconds
...
keep idauniq wave indager fqethnr education totwq5_bu_s disex scako smoking activity2 mynssec3 QoL AHA
...
* Save dataset with a new name
save GMMcovdataSES0317.dta
* One-to-one merge of data in memory with data.dta on participant ID and wave
merge 1:1 idauniq wave using data.dta, generate (merge_finstat)
* Keep if matched
keep if merge_finstat==3

tab sex if finstatw3=="C3CM"
tab disex if finstatw3=="C3CM"
* Generate a new variable duplicating the sex variable for the Wave 3 refreshment sample
gen sex3 = disex if finstatw3=="C3CM"
* Declare a panel dataset with participant ID "idauniq" and time variable "wave" 
tsset idauniq wave 
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 3 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward sex3, replace 

tab sex if finstatw4=="C4CM"
tab disex if finstatw4=="C4CM"
* Generate a new variable duplicating the sex variable for the Wave 4 refreshment sample
gen sex4 = disex if finstatw4=="C4CM"
* Declare a panel dataset with participant ID "idauniq" and time variable "wave" 
tsset idauniq wave 
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 4 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward sex4, replace 

tab sex if finstatw6==25
tab disex if finstatw6==25
* Generate a new variable duplicating the sex variable for the Wave 6 refreshment sample
gen sex6 = disex if finstatw6==25
* Declare a panel dataset with participant ID "idauniq" and time variable "wave" 
tsset idauniq wave 
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 6 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward sex6, replace 

tab sex if finstatw7==33
tab disex if finstatw7==33
* Generate a new variable duplicating the sex variable for the Wave 7 refreshment sample
gen sex7 = disex if finstatw7==33
* Declare a panel dataset with participant ID "idauniq" and time variable "wave" 
tsset idauniq wave 
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 7 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward sex7, replace 

tab sex if finstatw9==48
tab indsex if finstatw9==48
* Assign the number 0 if the participant is male at Wave 9
gen indsex9 = 0 if indsex==1 & wave==9
* Assign the number 1 if the participant is female at Wave 9
replace indsex9 = 1 if indsex==2 & wave==9
* Generate a new variable duplicating the sex variable for the Wave 9 refreshment sample
gen sex9 = indsex9 if finstatw9==48

* Generate a new variable duplicating the time-constant sex variable (assessed at Wave 2)
gen sexcons = sex 
* Replace the sex variable with values from the first wave of data collection for the refreshment samples
replace sexcons = sex3 if sexcons==. & sex3!=.
replace sexcons = sex4 if sexcons==. & sex4!=.
replace sexcons = sex6 if sexcons==. & sex6!=.
replace sexcons = sex7 if sexcons==. & sex7!=.
replace sexcons = sex9 if sexcons==. & sex9!=.

tab ethnicity if finstatw3=="C3CM"
tab fqethnr if finstatw3=="C3CM"
* Generate a new variable duplicating the ethnicity variable for the Wave 3 refreshment sample
gen eth3 = fqethnr if finstatw3=="C3CM"
* Declare a panel dataset with participant ID "idauniq" and time variable "wave" 
tsset idauniq wave 
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 3 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward eth3, replace 

tab ethnicity if finstatw4=="C4CM"
tab fqethnr if finstatw4=="C4CM"
* Generate a new variable duplicating the ethnicity variable for the Wave 4 refreshment sample
gen eth4 = fqethnr if finstatw4=="C4CM"
* Declare a panel dataset with participant ID "idauniq" and time variable "wave" 
tsset idauniq wave 
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 4 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward eth4, replace 

tab ethnicity if finstatw6==25
tab fqethnr if finstatw6==25
* Generate a new variable duplicating the ethnicity variable for the Wave 6 refreshment sample
gen eth6 = fqethnr if finstatw6==25
* Declare a panel dataset with participant ID "idauniq" and time variable "wave" 
tsset idauniq wave 
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 6 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward eth6, replace 

tab ethnicity if finstatw7==33
tab fqethnr if finstatw7==33
* Generate a new variable duplicating the ethnicity variable for the Wave 7 refreshment sample
gen eth7 = fqethnr if finstatw7==33
* Declare a panel dataset with participant ID "idauniq" and time variable "wave" 
tsset idauniq wave 
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 7 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward eth7, replace 

tab ethnicity if finstatw9==48
tab fqethnr if finstatw9==48
* Generate a new variable duplicating the ethnicity variable for the Wave 9 refreshment sample
gen eth9 = fqethnr if finstatw9==48

* Generate a new variable duplicating the time-constant ethnicity variable (assessed at Wave 2)
gen ethcons = ethnicity 
* Replace the ethnicity variable with values from the first wave of data collection for the refreshment samples
replace ethcons = eth3 if ethcons==. & eth3!=.
replace ethcons = eth4 if ethcons==. & eth4!=.
replace ethcons = eth6 if ethcons==. & eth6!=.
replace ethcons = eth7 if ethcons==. & eth7!=.
replace ethcons = eth9 if ethcons==. & eth9!=.

tab qualifications if finstatw3=="C3CM"
tab education if finstatw3=="C3CM"
* Generate a new variable duplicating the education variable for the Wave 3 refreshment sample
gen edu3 = education if finstatw3=="C3CM"
* Declare a panel dataset with participant ID "idauniq" and time variable "wave" 
tsset idauniq wave 
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 3 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward edu3, replace 

tab qualifications if finstatw4=="C4CM"
tab education if finstatw4=="C4CM"
* Generate a new variable duplicating the education variable for the Wave 4 refreshment sample
gen edu4 = education if finstatw4=="C4CM"
* Declare a panel dataset with participant ID "idauniq" and time variable "wave"
tsset idauniq wave 
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 4 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward edu4, replace 

tab qualifications if finstatw6==25
tab education if finstatw6==25
* Generate a new variable duplicating the education variable for the Wave 6 refreshment sample
gen edu6 = education if finstatw6==25
* Declare a panel dataset with participant ID "idauniq" and time variable "wave"
tsset idauniq wave
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 6 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward edu6, replace 

tab qualifications if finstatw7==33
tab education if finstatw7==33
* Generate a new variable duplicating the education variable for the Wave 7 refreshment sample
gen edu7 = education if finstatw7==33
* Declare a panel dataset with participant ID "idauniq" and time variable "wave"
tsset idauniq wave 
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 7 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward edu7, replace 

tab qualifications if finstatw9==48
tab education if finstatw9==48
* Generate a new variable duplicating the education variable for the Wave 9 refreshment sample
gen edu9 = education if finstatw9==48

* Generate a new variable duplicating the time-constant education variable (assessed at Wave 2)
gen educons = qualifications 
* Replace the education variable with values from the first wave of data collection for the refreshment samples
replace educons = edu3 if educons==. & edu3!=.
replace educons = edu4 if educons==. & edu4!=.
replace educons = edu6 if educons==. & edu6!=.
replace educons = edu7 if educons==. & edu7!=.
replace educons = edu9 if educons==. & edu9!=.

tab nssec3 if finstatw3=="C3CM"
tab mynssec3 if finstatw3=="C3CM"
* Generate a new variable duplicating the occupational class variable for the Wave 3 refreshment sample
gen occ3 = mynssec3 if finstatw3=="C3CM"
* Declare a panel dataset with participant ID "idauniq" and time variable "wave"
tsset idauniq wave 
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 3 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward occ3, replace 

tab nssec3 if finstatw4=="C4CM"
tab mynssec3 if finstatw4=="C4CM"
* Generate a new variable duplicating the occupational class variable for the Wave 4 refreshment sample
gen occ4 = mynssec3 if finstatw4=="C4CM"
* Declare a panel dataset with participant ID "idauniq" and time variable "wave"
tsset idauniq wave 
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 4 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward occ4, replace 

tab nssec3 if finstatw6==25
tab mynssec3 if finstatw6==25
* Generate a new variable duplicating the occupational class variable for the Wave 6 refreshment sample
gen occ6 = mynssec3 if finstatw6==25
* Declare a panel dataset with participant ID "idauniq" and time variable "wave"
tsset idauniq wave 
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 6 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward occ6, replace 
* Save dataset with a new name
save intermediate.dta
clear

* Recode the long form occupational class variable at Wave 7 into the three-class classification
use idauniq NSSEC using wave_7_elsa_data.dta
* Generate a new variable called wave and assign the number 7 to each observation (to designate Wave 7)
gen wave = 7
* Replace variable as missing for any missing cases (coded as negative numbers in the ELSA dataset)
replace NSSEC = . if NSSEC==-1
* Replace variable with the number 0 if the participant's current or most recent occupation was coded as: Lower supervisory and technical occupations; or Semi-routine occupations; or Routine occupations
replace NSSEC = 0 if inlist(NSSEC,10,11.1,11.2,12.1,12.2,12.3,12.4,12.5,12.6,12.7,13.1,13.2,13.3,13.4,13.5)
* Replace variable with the number 1 if the participant's current or most recent occupation was coded as: Intermediate occupation; or Small employers and own account workers
replace NSSEC = 1 if inlist(NSSEC,9.1,9.2,8.1,8.2,7.1,7.2,7.3,7.4)
* Replace variable with the number 2 if the participant's current or most recent occupation was coded as: Higher managerial, administrative and professional occupations; or Lower managerial, administrative and professional occupations
replace NSSEC = 2 if inlist(NSSEC,3.1,3.2,3.3,3.4,4.1,4.2,4.3,4.4,6,5)
* Save dataset with a new name
save wave7nssec.dta

* Use dataset for creating the time-constant variables
use intermediate.dta 
* Drop the long form occupational class variable
drop NSSEC
* One-to-one merge of data in memory with wave7nssec.dta on participant ID and wave
merge 1:1 idauniq wave using wave7nssec.dta, generate (merge_nssec)
* Restore sample size
keep if AHA!=.

tab nssec3 if finstatw7==33
tab NSSEC if finstatw7==33 & wave==7
* Generate a new variable duplicating the occupational class variable for the Wave 7 refreshment sample
gen occ7 = NSSEC if finstatw7==33
* Declare a panel dataset with participant ID "idauniq" and time variable "wave"
tsset idauniq wave 
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 7 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward occ7, replace 

tab nssec3 if finstatw9==48
tab mynssec3 if finstatw9==48
* Generate a new variable duplicating the occupational class variable for the Wave 9 refreshment sample
gen occ9 = mynssec3 if finstatw9==48

* Generate a new variable duplicating the time-constant occupational class variable (assessed at Wave 2)
gen occcons = nssec3 
* Replace the occupational class variable with values from the first wave of data collection for the refreshment samples
replace occcons = occ3 if occcons==. & occ3!=.
replace occcons = occ4 if occcons==. & occ4!=.
replace occcons = occ6 if occcons==. & occ6!=.
replace occcons = occ7 if occcons==. & occ7!=.
replace occcons = occ9 if occcons==. & occ9!=.

tab wealth if finstatw3=="C3CM"
tab totwq5_bu_s if finstatw3=="C3CM"
* Generate a new variable duplicating the wealth variable for the Wave 3 refreshment sample
gen wealth3 = totwq5_bu_s if finstatw3=="C3CM"
* Declare a panel dataset with participant ID "idauniq" and time variable "wave"
tsset idauniq wave 
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 3 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward wealth3, replace 

tab wealth if finstatw4=="C4CM"
tab totwq5_bu_s if finstatw4=="C4CM"
* Generate a new variable duplicating the wealth variable for the Wave 4 refreshment sample
gen wealth4 = totwq5_bu_s if finstatw4=="C4CM"
* Declare a panel dataset with participant ID "idauniq" and time variable "wave"
tsset idauniq wave 
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 4 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward wealth4, replace 

tab wealth if finstatw6==25
tab totwq5_bu_s if finstatw6==25
* Generate a new variable duplicating the wealth variable for the Wave 6 refreshment sample
gen wealth6 = totwq5_bu_s if finstatw6==25
* Declare a panel dataset with participant ID "idauniq" and time variable "wave"
tsset idauniq wave 
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 6 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward wealth6, replace 

tab wealth if finstatw7==33
tab totwq5_bu_s if finstatw7==33
* Generate a new variable duplicating the wealth variable for the Wave 7 refreshment sample
gen wealth7 = totwq5_bu_s if finstatw7==33
* Declare a panel dataset with participant ID "idauniq" and time variable "wave"
tsset idauniq wave 
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 7 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward wealth7, replace 

tab wealth if finstatw9==48
tab totwq5_bu_s if finstatw9==48
* Generate a new variable duplicating the wealth variable for the Wave 9 refreshment sample
gen wealth9 = totwq5_bu_s if finstatw9==48

* Generate a new variable duplicating the time-constant wealth variable (assessed at Wave 2)
gen wealthcons = wealth 
* Replace the wealth variable with values from the first wave of data collection for the refreshment samples
replace wealthcons = wealth3 if wealthcons==. & wealth3!=.
replace wealthcons = wealth4 if wealthcons==. & wealth4!=.
replace wealthcons = wealth6 if wealthcons==. & wealth6!=.
replace wealthcons = wealth7 if wealthcons==. & wealth7!=.
replace wealthcons = wealth9 if wealthcons==. & wealth9!=.

tab age if finstatw3=="C3CM"
tab indager if finstatw3=="C3CM"
* Generate a new variable duplicating the age variable for the Wave 3 refreshment sample
gen agew3 = indager if finstatw3=="C3CM" & wave==3

tab age if finstatw4=="C4CM"
tab indager if finstatw4=="C4CM"
* Generate a new variable duplicating the age variable for the Wave 4 refreshment sample
gen agew4 = indager if finstatw4=="C4CM" & wave==4

tab age if finstatw6==25
tab indager if finstatw6==25
* Generate a new variable duplicating the age variable for the Wave 6 refreshment sample
gen agew6 = indager if finstatw6==25 & wave==6

tab age if finstatw7==33
tab indager if finstatw7==33
* Generate a new variable duplicating the age variable for the Wave 7 refreshment sample
gen agew7 = indager if finstatw7==33 & wave==7

tab age if finstatw9==48
tab indager if finstatw9==48
* Generate a new variable duplicating the age variable for the Wave 9 refreshment sample
gen agew9 = indager if finstatw9==48 & wave==9

* Generate a new variable duplicating the age variable for the Wave 3 refreshment sample
gen agew3cons = agew3 if wave==3
* Declare a panel dataset with participant ID "idauniq" and time variable "wave"
tsset idauniq wave
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 3 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward agew3cons, replace

* Generate a new variable duplicating the age variable for the Wave 4 refreshment sample
gen agew4cons = agew4 if wave==4
* Declare a panel dataset with participant ID "idauniq" and time variable "wave"
tsset idauniq wave
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 4 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward agew4cons, replace

* Generate a new variable duplicating the age variable for the Wave 6 refreshment sample
gen agew6cons = agew6 if wave==6
* Declare a panel dataset with participant ID "idauniq" and time variable "wave"
tsset idauniq wave
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 6 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward agew6cons, replace

* Generate a new variable duplicating the age variable for the Wave 7 refreshment sample
gen agew7cons = agew7 if wave==7
* Declare a panel dataset with participant ID "idauniq" and time variable "wave"
tsset idauniq wave
* Carryforward observations with respect to the time variable "wave" (i.e., from Wave 7 to the follow-up waves available for that participant) by participant ID
bysort idauniq: carryforward agew7cons, replace

* Generate a new variable duplicating the time-constant age variable (assessed at Wave 2)
gen agewcons = age
* Replace the age variable with values from the first wave of data collection for the refreshment samples
replace agewcons = agew3cons if agewcons==. & agew3cons!=.
replace agewcons = agew4cons if agewcons==. & agew4cons!=.
replace agewcons = agew6cons if agewcons==. & agew6cons!=.
replace agewcons = agew7cons if agewcons==. & agew7cons!=.
replace agewcons = agew9 if agewcons==. & agew9!=.

* Save dataset with a new name
save timeconstant.dta

* Keep variables required for analyses
keep idauniq wave sexcons educons wealthcons agewcons ethcons occcons QoL AHA
...
* Save dataset with a new name
save GMMcovdatawideSES0323.dta
sum QoL9, d
...
gen mediumed = 0 if inlist(educons,0,2)
replace mediumed = 1 if educons == 1
* High education (coded as 1) versus low or medium education (coded as 0)
gen highed = 0 if inlist(educons,0,1)
replace highed = 1 if educons == 2
* Occupational class
* Intermediate occupations (coded as 1) versus lower or higher occupations (coded as 0)
gen mediumocc = 0 if inlist(occcons,0,2)
replace mediumocc = 1 if occcons == 1
* Higher occupations (coded as 1) versus lower or intermediate occupations (coded as 0)
gen highocc = 0 if inlist(occcons,0,1)
replace highocc = 1 if occcons == 2
* Wealth
* 2nd quintile (coded as 1) versus 1st, 3rd, 4th, or 5th quintile (coded as 0)
gen quint2 = 0 if inlist(wealthcons,1,3,4,5)
replace quint2 = 1 if wealthcons == 2
* 3rd quintile (coded as 1) versus 1st, 2nd, 4th, or 5th quintile (coded as 0)
gen quint3 = 0 if inlist(wealthcons,1,2,4,5)
replace quint3 = 1 if wealthcons == 3
* 4th quintile (coded as 1) versus 1st, 2nd, 3rd, or 5th quintile (coded as 0)
gen quint4 = 0 if inlist(wealthcons,1,2,3,5)
replace quint4 = 1 if wealthcons == 4
* 5th quintile (coded as 1) versus 1st, 2nd, 3rd, or 4th quintile (coded as 0)
gen quint5 = 0 if inlist(wealthcons,1,2,3,4)
replace quint5 = 1 if wealthcons == 5
* Save dataset with a new name
save GMMcovbinarySESdu0323.dta
* Convert Stata data into a data file and Mplus input file
stata2mplus using GMMcovbinarySESdu0323.dta
clear

* Import posterior probabilities of class membership and most likely class membership following step 1 of the three-step manual GMM procedure
import excel "", sheet("") firstrow
* Convert Stata data into a data file and Mplus input file
stata2mplus using step2c3manual0323
* Save dataset with a new name
save step2c3manual0323.dta
...
* Use timeconstant.dta dataset
use timeconstant.dta
* Count total number of participants and observations
unique idauniq
* 14,755 individuals, 66,133 observations
...
* Overwrite dataset, by replacing the previously saved file
save ROCdata0323.dta, replace
* Drop unnecessary variable
drop QoL
* Reshape data into wide format for observations identified by participant ID and add wave as an identifying time period
reshape wide AHA QoLbinary, j(wave) i (idauniq)
* Save dataset with a new name
save ROC92wide0323.dta
...
rocreg QoLbinary3 AHA2, probit ml ctrlcov(sexcons) ctrlmodel(linear) cluster(idauniq)
* Quality-of-life at Wave 6
rocreg QoLbinary6 AHA2, probit ml ctrlcov(sexcons) ctrlmodel(linear) cluster(idauniq)
* Quality-of-life at Wave 9
rocreg QoLbinary9 AHA2, probit ml ctrlcov(sexcons) ctrlmodel(linear) cluster(idauniq)
clear
...
mixed AHA indager i.sexcons i.ethcons i.scako i.smoking i.activity2 || idauniq:
...
mixed AHA indager i.sexcons i.ethcons i.scako i.smoking i.activity2 || idauniq: wave
* Store estimates for later use
...
* Use GMMcovbinarySES0323.dta dataset
use GMMcovbinarySES0323.dta
* Drop observations with missing data on socio-economic or demographic covariates
drop if sexcons==. | educons==. | wealthcons==. | agewcons==. | ethcons==. | occcons ==. 
* Count total number of participants and observations
unique idauniq
* 11,566 individuals
* Keep necessary variables
keep idauniq sexcons educons wealthcons agewcons ethcons occcons
* Save dataset with a new name
save descbyclassGMMcovdataSESwide0325.dta
clear

* Import posterior probabilities of class membership and most likely class membership following step 3 of the three-step manual GMM procedure
import excel "", sheet("") firstrow
* Save dataset with a new name
save manualstep3c3.dta
clear

* Use descbyclassGMMcovdataSESwide0325.dta dataset
use descbyclassGMMcovdataSESwide0325.dta
* One-to-one merge of data in memory with manualstep3c3.dta on participant ID
merge 1:1 idauniq using manualstep3c3.dta, generate (merge_posterior)
* Overwrite dataset, by replacing the previously saved file
save descbyclassGMMcovdataSESwide0325.dta, replace
...
tab wealthcons if n==1
tab wealthcons if n==2
tab wealthcons if n==3